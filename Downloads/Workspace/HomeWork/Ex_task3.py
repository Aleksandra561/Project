...
countries = open (dirfile+'countries.csv','w', encoding='utf-8')
for i in my_list:
...
for line in open (Export, encoding='utf-8'):
    ...
    if len(mainline)>11:
        ...
        if country_id!=-1:
            state_city = f"{index}\t{country_id}\t{state}\t{city}"
    if state_city:
...
for state_city in my_list:
    ...
    sity_state_id += 1 
state_cities = open (dirfile+'state_cities.csv','w', encoding='utf-8')
for i in my_list:
    index, country, state, city = i.split('\t')
    #country_id = country_dict_rev.get(country, -1)
    encoding='utf-8'
    state_cities.write(f"{i}\n")
state_cities.close()
print(f"{i}\n")
...
for line in open (Export, encoding='utf-8'):
    ...
    if len(mainline)>21:
        ...
        street = mainline[7].strip()
        index = mainline[11].strip()
        lan = mainline[20].strip()
        lon = mainline[21].strip()
        country = mainline[9].strip()
        markets = f"{FMID}\t{market_name}\t{street}\t{index}\t{lan}\t{lon}\t{country}"
...
for i in my_list:
   FMID,market_name,street, index,lan,lon,country = i.split('\t') 
   country_id = country_dict_rev.get(country, -1)
   ...
   markets.write(f"{FMID}\t{market_name}\t{street}\t{index}\t{lan}\t{lon}\t{country_id}\n")  
markets.close()
